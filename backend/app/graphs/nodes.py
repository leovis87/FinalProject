...
# ============================================
# 1. ...?
def _create_eng_gent(topic: str,
                     logs: dict,
                     persona: dict,
                     next_speaker: str,
                     phase: int,
    ...
    description:
        - 한국어 -> 영어로 비용 최적화
        - DB에서 topic, logs, persona, next_speaker, phase, guide_msg 를 인자로 받아야 함.
    '''
    agent = create_agent(
        model = llm_c_configured,
        tools = [TAVILY_SEARCH],
...
# 2. 방 생성 시 DB RAG 유사도에 없는 주제일 시 호출되는 Agent
def _create_recommend_topic_agent():
    '''
    description:
        - 방 생성 시 DB RAG 검색 후, Topic이 없을 때 추천
        - 
    '''

    return {}

# 3. 사회자 AI가 생성한 내용이 적절한 평가인지 check
# 📌 설명:
# Agent + search tool
# - 정확한 채점 (미묘한 차이 구분)
# - Fact-check 포함
# - 구체적 근거 제시
def _create_agent_checker_agent():
    '''
    description:
        - 채점용 agent의 출력을 check하는 agent
        - High perfomance Agent (Gemini_Pro or Claude opus_4) + search tool
    '''
    agent = create_agent(
        model = llm_g_real_non_harm,
        tools = [TAVILY_SEARCH],
        system_prompt = SystemMessage(
            content = [
                {
...
오늘 날짜: {datetime.now().strftime('%Y년 %m월 %d일')}"""
                },
                {
                    ...
                    'text': f"""
[Important]
1. 반드시 아래 Output Format JSON 형식만 출력할 것.
2. 추가 설명이나 마크다운은 포함하지 말 것.
3. 모든 메트릭에 대해서 평가를 해야함.
4. 각 메트릭에서 내용 평가 시 search tool을 사용하여 fact-check 할 것.
"""
                },
                {
                    'type': 'text',
                    'text': f"""
[Role]
당신은 LLM as a Judge입니다. AI 에이전트의 응답을 객관적으로 평가합니다.

[Task]
사용자의 쿼리와 AI 에이전트의 응답을 평가하고 점수를 부여합니다.
1. 사실적 정확성 메트릭 검증 시 fact-check (검색 필수)
2. 구체적 근거와 출처 제시

[채점 기준]
1. 할루시네이션 (수준: 0-5)
   - 수준 5: 할루시네이션이 전혀 없음. 모든 정보가 제공된 컨텍스트에 기반함
   - 수준 4: 대부분 정확하나 사소한 추론이 포함됨
   - 수준 3: 일부 검증되지 않은 정보가 포함됨
   - 수준 2: 상당한 할루시네이션이 존재함
   - 수준 1: 대부분의 정보가 근거 없음

2. 관련성 (수준: 0-5)
   - 수준 5: 질문에 완벽하게 부합하는 답변
   - 수준 4: 질문과 관련성이 높으나 일부 불필요한 정보 포함
   - 수준 3: 질문과 관련있으나 핵심을 벗어남
   - 수준 2: 질문과 부분적으로만 관련됨
   - 수준 1: 질문과 거의 무관함

3. 충실도 (수준: 0-5)
   - 수준 5: 제공된 컨텍스트에 100% 기반한 답변
   - 수준 4: 대부분 컨텍스트에 기반하나 약간의 추론 포함
   - 수준 3: 컨텍스트와 외부 지식이 혼재됨
   - 수준 2: 컨텍스트보다 외부 지식에 의존
   - 수준 1: 컨텍스트를 거의 무시함

4. 사실적 정확성 (수준: 0-5)
   - 수준 5: 모든 사실이 정확함
   - 수준 4: 대부분 정확하나 사소한 오류 존재
   - 수준 3: 일부 부정확한 정보 포함
   - 수준 2: 상당수의 부정확한 정보
   - 수준 1: 대부분의 정보가 부정확함
...
# 🛠️ 수정 중인 노드 모음
# ============================================
# 입력된 topic을 DB에서 RAG로 확인 => 개별 함수로 바꿔야함...
def check_topic_node(state: DebateState) -> DebateState:
    '''
    description:
        - 입력된 topic이 Vector DB에 있는지 확인 (RAG 검색)
        - 있으면 is_topic_valid = True, 없으면 False
    '''
    print(f"\n{'='*50}")
    print(f"[🔍 주제 검증 Node] '{state['topic']}' DB 검색 중...")
    print(f"{'='*50}")

    # ---------------------------------------------------------
    # TODO: 실제 Vector DB(Pinecone/Chroma) 검색 로직 연결
    # ---------------------------------------------------------
    # 예시: mock logic (임시)
    # 실제로는 retriever.invoke(state['topic']) 결과가 비었는지 체크
    
    # (가정) 'DB'라는 단어가 포함되면 찾은 것으로 간주, 아니면 못 찾음
    # 실전에서는 threshold(유사도 점수) 0.7 이상이면 True 등으로 구현
    is_found = False # 테스트를 위해 일단 '못 찾음'으로 설정 (추천 로직 확인용)

    # 만약 RAG 검색 성공 시:
    # is_found = True

    if is_found:
        print("✅ DB에서 관련 자료를 찾았습니다. 토론을 바로 시작합니다.")
        logging.info("✅ DB에서 관련 자료를 찾았습니다. 토론을 바로 시작합니다.")
        return {
            'is_topic_valid': True
        }

    else:
        print("❌ DB에 관련 자료가 부족합니다. 추천 모드로 진입합니다.")
        logging.info("❌ DB에 관련 자료가 부족합니다. 추천 모드로 진입합니다.")
        return {
            'is_topic_valid': False
        }

# Topic 추천 함수 => 개별 함수로 바꿔야함...
def recommend_topic_node(state: DebateState,
                         config: RunnableConfig) -> DebateState:
    '''
    description:
        - RAG 검색 실패 시 실행
        - 주제가 모호할 때, Search Tool 검색 후, 3가지 대안 주제 + 설명 + 각각의 입론 추천
        - Config를 함수 내부에서 덮어씌워 Temp(창의성)을 높임
    '''
    print(f"\n{'='*50}")
    print(f"[Topic 추천 Node 실행]")
    print(f"{'='*50}")
    
    # 1. Search tool 선택 및 실행
    search_tool = get_search_tool(use_tavily = True)
    search_query = f"토론 주제 추천: {state['topic']} 관련 최신 이슈 및 찬반 쟁점 분석"
    search_result = search_tool.invoke(search_query)

    # ⭐ 먼저 기존 Agent에 pydantic을 추가 설정
    # with_structured_output으로 처리되는 LLM 내부 과정
    # {
    #     "type": "object",
    #     "properties": {
    #         "recommendations": {
    ...
    #             "con_arg": {"type": "string"}
    #             },
    #             "required": ["topic", "pro_arg", "con_arg"]
    #         }
    #         }
    #     },
    #     "required": ["recommendations"]
    # }
    structured_agent = llm_g_real_non_harm.with_structured_output(RecommendationResponse)


    # 2. 전역의 config 복사 후, '추천용 설정'으로 변경
    recommend_config = config.copy() if config else {}

    # 'configurable' 키가 없으면 생성
    if 'configurable' not in recommend_config:
        recommend_config['configurable'] = {}

    # 추천을 위해 Temp(창의성)을 높게 부여 (0.8) & 모델 변경
    # pydantic을 추가 설정한 Agent에 하기 설정 변경 됨!!
    recommend_config['configurable']['temp'] = 0.8
    recommend_config['configurable']['model'] = GEMINI_MODELS['flash']

    # 3. 프롬프트 작성 (JSON 포맷 필요없음 => pydantic으로 잡힘)
    prompt = f"""
사용자가 입력한 키워드:
'{state['topic']}'
...
위 내용을 바탕으로 토론하기 좋은 구체적인 주제 3가지와 각 팀의 입론을 2가지 추천해 줘.

[작성 요구사항]
1. 주제명: 명확하고 간결하게
2. 설명: 이 주제가 왜 지금 중요한지 1문장으로 요약
3. 입론: 찬성/반대 각각 핵심 주장 2가지씩 포함
4. 언어: 한국어
"""
    
    # 4. 실행 (tool + 설정 + 프롬프트)
    try:
        # ⭐ 여기서 config를 넣어주면, 내부의 기본 LLM까지 설정이 전달 됨.
        result_obj = structured_agent.invoke(prompt,
                                             config = recommend_config)
        
        # Pydantic 객체를 List[dict] 형태로 변환
        # 💡 structured_agent의 response가 RecommendationResponse(BaseModel)이므로,
        #    .recommendations 클래스를 반환함!!

        # recommendations = [
        #     rec.dict()                        # 3. 각 TopicRecommendation 객체를 dict로 변환해라 ({'topic':...})
        #     for rec in                        # 2. 그 리스트 안의 요소(rec)를 하나씩 꺼내서
        #     result_obj.recommendations        # 1. result_obj 안에 있는 'recommendations' 리스트에서
        # ]
        recommendations = [rec.dict() for rec in result_obj.recommendations]

        # 로그 출력 확인
        for i, rec in enumerate(recommendations):
            print(f"[{i + 1}] {rec['topic']} ({rec['description']})")
            print(f"    [찬성]: {rec['pro_args']}")
            print(f"    [반대]: {rec['con_args']}")
            logging.info(f"[{i + 1}] {rec['topic']} ({rec['description']})")
            logging.info(f"    [찬성]: {rec['pro_args']}")
            logging.info(f"    [반대]: {rec['con_args']}")
...
# ============================================
# 기본 대화 Chain LLM 노드
def chat_node(state: DebateState,
              config: RunnableConfig) -> DebateState:
    '''
    description:
        - 채팅 Node: 일반 chain LLM 대화
    '''
    print(f'[채팅 Node] 일반 대화 처리...')
    ...
    # ============================================
    # 1. 설정값 가져오기 (가독성을 위해 변수로 명확하게 꺼내옴)
    user_config = config.get('configurable', {}) if config else {}

    # 2. 'is_stream' 키가 무엇인지 몰라도, 변수명으로 `직관적으로` 알 수 있게 설정
    enable_streaming = user_config.get('is_stream', False)

    # 3-1. 콜백(Callback) 명시적 설정
    #   ⭐ 기존 callback 보존 + 새 내용 추가   
    existing_callbacks = config.get('callbacks', []) if config else []
    new_callbacks = []

    # 3-2. 기존 콜백이 있으면 리스트로 추출 -> 옮겨 담기
    if existing_callbacks:
        if isinstance(existing_callbacks, list):
            new_callbacks.extend(existing_callbacks)

        elif hasattr(existing_callbacks, 'handlers'): # CallbackManager인 경우
    ...
    # 4. 스트리밍이 켜져있으면 -> 핸들러를 리스트에 넣음
    #   ⭐ Streaming 핸들러 + stream 옵션이 둘다 켜져 있어야 invoke에서도 stream 됨
    if enable_streaming:
        print(">> 스트리밍 모드 ON: 실시간으로 답변을 출력합니다.")
        new_callbacks.append(StreamingStdOutCallbackHandler())
    
    # 5. 기존 config (workflow app을 실제로 실행 시, 기입 받는 config) 복사,
    #   실행용 config 새로 만들기 ->
    #   callbacks 파라미터에 new_callbacks list를 넣음 (기존 + 추가)
...
# 기본 마지막 응답 노드
def response_node(state: DebateState) -> DebateState:
    '''
    description:
        - 최종 응답 Node: 검색 결과를 정리
    '''
    print(f'\n[응답 Node] 최종 응답 생성...')

    # 검색 결과가 있고, 비어있지 않을 때만
    if state.get('search_result') and state['search_result'].strip():
        final_response = f'''
📅 기준: {state['current_date']}
...
'''.strip()
        ...
        # 일반 대화 응답 사용
        final_response = state.get('final_response', '응답 생성 실패')
...
# 📌 설명:
# search_agent_node = 05에서 배운 create_agent 사용!
# - 진짜 DuckDuckGo 검색
# - 현재 날짜 포함
# - system_prompt로 동작 제어

# 기본 Chain LLM 사회자 AI 노드
def moderator_chain_node(state: DebateState,
                         config: RunnableConfig) -> DebateState:
    '''
    description:
        - 무료 버젼: chian llm 사용
    ...
    # 전체 대화 내용 문자열로 변환
    debate_content = '\n\n'.join([
        f"[{msg['user_name']}({msg['role']})] {msg['content']}"
        for msg in state['messages']
    ])
